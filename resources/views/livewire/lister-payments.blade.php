                        ...
                        <tbody>
                            @foreach ($payments as $payment)
                                <tr class="border-t border-slate-200 text-sm text-slate-700 dark:border-slate-800 dark:text-slate-300">
                                    ...
                                    <td class="px-5 py-4">{{ $payment->renter->name }}</td>
                                    <td class="px-5 py-4 text-right font-bold text-slate-950 dark:text-white">&#8369;{{ number_format($payment->total_price, 2) }}</td>
                                    <td class="px-5 py-4 text-right">&#8369;{{ number_format((float) $payment->paid_amount, 2) }}</td>
                                    ...
                                    <td class="px-5 py-4">
                                        @if ($payment->payment_status !== 'fully_paid')
                                            <div class="flex justify-end gap-2">
                                                <input type="number" step="0.01" min="0.01" wire:model.defer="paymentAmounts.{{ $payment->id }}" class="w-28 rounded-md border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="Amount">
                                            ...
                                            @error('paymentAmounts.' . $payment->id)
                                                <p class="mt-1 text-right text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                                            @enderror
                                        @else
                                            <p class="text-right text-xs font-semibold text-emerald-700 dark:text-emerald-300">Complete</p>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
